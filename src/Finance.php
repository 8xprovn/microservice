...
class Finance
{
    ...
    public function __construct() {
     ...
     public function getInvoicesIncludeDetails($params) {
        $whereArr = \Arr::only($params, ['invoice_id', 'employee_id']);
        $filter = [];
        foreach($whereArr as $k => $v){
            if (is_null($v)) continue;
        ...
        $response = \Http::withToken(env('API_MICROSERVICE_TOKEN',''))->get($this->_url.'/invoices',['filter' => json_encode([
            ...
            'include' => [
                [
                    ...
                    // 'scope' => [
                    //     'fields'=> ['detail_id','invoice_id','relate_id','note','quantity','sub_total','total','relate_name'],
                    // ]
                ]
            ],
            ])]);
    ...
    public function getInvoicesIncludePayments($params) {
        $whereArr = \Arr::only($params, ['invoice_id', 'employee_id']);
        $filter = [];
        foreach($whereArr as $k => $v){
            if (is_null($v)) continue;
        ...
        $response = \Http::withToken(env('API_MICROSERVICE_TOKEN',''))->get($this->_url.'/invoices',['filter' => json_encode([
            ...
            'include' => [
                [
                    ...
                    // 'scope' => [
                    //     'fields'=> ['payment_id','invoice_id','amount','method','status','note','employee_id','accountant_id'],
                    // ]
                ]
            ],
            ])]);
